...
public class MyVmxFile implements VmxFile {
    ...
    public MyVmxFile(File file) throws VmxException {
        ...
        // first 24 bytes: fixed header
        if ((fileBytes[0] & 0xFF) != 'v' || (fileBytes[1] & 0xFF) != 'm' || (fileBytes[2] & 0xFF) != 'x' ||
                (fileBytes[3] & 0xFF) != '\0') {
            throw new VmxException("File signature incorrect " + file.getAbsolutePath());
        }
        int extCount = fileBytes[4] & 0xFF;

        short major = readShort(fileBytes, 8);
        short minor = readShort(fileBytes, 10);
        int buildNum = readInt(fileBytes, 12);
        version = new Version(major, minor, buildNum);
        flags = fileBytes[6] & 0xFF;
        entryOffset = readInt(fileBytes, 20);

        // next 32 bytes: section headers
        int textOffset = readInt(fileBytes, 24);
        int textSize = readInt(fileBytes, 28);
        int rodataOffset = readInt(fileBytes, 32);
        int rodataSize = readInt(fileBytes, 36);
        int dataOffset = readInt(fileBytes, 40);
        int dataSize = readInt(fileBytes, 44);
        int bssOffset = readInt(fileBytes, 48);
        int bssSize = readInt(fileBytes, 52);

        // (12 bytes * extCount) (extension headers)
        int headerSize = 24 + 32 + (12 * extCount);

        text   = Arrays.copyOfRange(fileBytes, headerSize + textOffset, headerSize + textOffset + textSize);
        rodata = Arrays.copyOfRange(fileBytes, headerSize + rodataOffset, headerSize + rodataOffset + rodataSize);
        data   = Arrays.copyOfRange(fileBytes, headerSize + dataOffset, headerSize + dataOffset + dataSize);
        bss    = new byte[bssSize];
        ...
        for (int i = 0; i < extCount; i++) {
            ...
            byte[] data = Arrays.copyOfRange(fileBytes, headerSize + offset, headerSize + offset + size);
            MyVmxExt ext = new MyVmxExt(type, extFlag, data);
            extensions.add(ext);
    ...
    private void writeInt(byte[] dest, int offset, int value) {
    ...
    private void writeShort(byte[] dest, int offset, short value) {
        dest[offset]     = (byte) value;
        dest[offset + 1] = (byte) (value >>> 8);
    }
}
